...
class Experiment():
    EYE, WHEEL = 0,1
    def __init__(self, name, animal, save_path=r'C:\Users\deverett\Desktop'):
    ...
    def run(self):
        ...
        self.data_eye = np.zeros(self.plot_n)
        self.data_wheel = np.zeros(self.plot_n)
        q = 0
    ...
    def determine_eyelid(self):
        return np.std(self.data_eye[-self.window_eye:]) > self.thresh_eye
    ...
    def extract(self, fr):
        ...
        return dp/self.mask_flat.sum(axis=-1)
    def acquire_masks(self):
        im1 = self.cam1.get()
        pl.imshow(im1, cmap='gray')
        pl.title('Select Eye')
        pts_eye = pl.ginput(timeout=1e9)
        ...
        pts_wheel = pl.ginput(timeout=1e9)
